...
if (!command) {
  console.error("Usage: npm run cli -- <ingest|rules|approve-rules|qc|lineage|demo> ...");
  process.exit(1);
}

process.argv = [process.argv[0] ?? "node", process.argv[1] ?? "cli", ...rest];

if (command === "ingest") await import("./ingest.js");
else if (command === "rules") await import("./generate-rules.js");
